...
| `HealOverlay` | Floating "+N" / "~N" recovered HP, to the right of the health bar (real max HP via NPCManager + Hiscores, always on) | `addHeal(actor, amount, estimate)` on heal detection each tick | `healDisplayMode` (Off / Everyone / Me / Opponents / Self+opp) + `healSize` | **Heal +12**, **Heal ~30 (est)** |
...
| `AttackTimerOverlay` | Depleting cooldown PIE (green→red) above the head/skull until the actor can attack again | `AttackCooldownService` [start,ready] window (attack opens it; eat extends) | `attackTimerSelf/Opponents/Others` | — (needs a live attack) |
